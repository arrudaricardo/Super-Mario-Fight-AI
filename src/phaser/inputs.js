    ...
    if (cursors.space.isDown) {
      marioHammer(() => {
        ...
        if (mario.data.values.facing === "right") {
          mario.play("m-hammer-right", false).setCrop(0, 1, 43, 42);
        }
        if (mario.data.values.facing === "left") {
          mario.play("m-hammer-left", true).setCrop(2, 2, 44, 42);
        }
      });
    }
    //jumping
    ...
    // walk right
      ...
      if (mario.getData("hammerCompleted")) {
        ...
        if (!mario.body.touching.down && mario.body.facing === 11) {
          ...
          if (mario.data.values.facing === "left") {
            mario.play("m-jump-left", 1).setCrop(0, 1, 43, 42);
          }
          // }
        }
        // mario is falling
        if (!mario.body.touching.down && mario.body.facing === 12) {
          // if (currentAction(mario) !== 'hammer'){
          if (mario.data.values.facing === "right") {
            mario.play("m-falling-right", 1).setCrop(0, 1, 43, 42);
          }
          if (mario.data.values.facing === "left") {
            mario.play("m-falling-left", 1).setCrop(0, 1, 43, 42);
          }
          // }
        }
      }
    ...
    // walk right
      ...
      if (luigi.getData("hammerCompleted")) {
        ...
        if (!luigi.body.touching.down && luigi.body.facing === 11) {
          // check prevFacing
          if (luigi.data.values.facing === "right") {
            luigi.play("l-jump-right", 1).setCrop(0, 1, 43, 42);
          }
          if (luigi.data.values.facing === "left") {
        ...
        if (!luigi.body.touching.down && luigi.body.facing === 12) {
          if (luigi.data.values.facing === "left") {
            luigi.play("l-falling-right", 1).setCrop(0, 1, 43, 40);
          }
          if (luigi.data.values.facing === "left") {
            luigi.play("l-falling-left", 1).setCrop(0, 1, 43, 42);
          }
        }
      }
    }
